...
p = ggplot(data, aes(x=rate, fill=type)) + 
          ...
          scale_fill_manual(values = c(hue_pal()(3)[2],hue_pal()(3)[3])) +
          guides(fill = guide_legend(title = "Transition Rates")) +
          geom_vline(xintercept = mean(log$rate_01), linetype = "longdash", color=hue_pal()(3)[3]) +
          geom_vline(xintercept = mean(log$rate_10), linetype = "longdash", color=hue_pal()(3)[2]) 
